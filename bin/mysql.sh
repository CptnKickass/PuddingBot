...
# 
# mysql -u ${sqlUser} -p${sqlPass} -e "show databases"
# 
...
# 
# mysql -u ${sqlUser} -p${sqlPass} -e "use ${sqlDBname}"
# 
# ${?} Returns 0 if good, 1 if bad
# ----------------------------------------------------
...
# 
# mysql -u ${sqlUser} -p${sqlPass} -e "use ${sqlDBname}; INSERT INTO users VALUES ('${sqlNuh}','${sqlNick},'${sqlSeen}','${sqlSeenSaid}','${sqlSeenSaidIn}','0');"
# 
# Returns nothing
# ----------------------------------------------------
...
echo "This will create the needed mysql user and database for PuddingBot."
echo "To do this, a mysql account with all privilages (usually root) is needed."
...
read -p "Press [Enter] to continue, or Ctrl+C to quit." null
echo ""
read -p "Please enter mysql root (or all privs) username: " sqlRootUser
read -p "Please enter ${sqlRootUser}'s password: " sqlRootPass
echo ""

echo "Testing for good username and password..."
mysql -u ${sqlRootUser} -p${sqlRootPass} -e "show databases" > /dev/null 2>&1
if [ "${?}" -eq "0" ]; then
...
mysql -u ${sqlRootUser} -p${sqlRootPass} -e "CREATE USER '${sqlUser}'@'localhost' IDENTIFIED BY  '${sqlPass}';"
if [ "${?}" -eq "0" ]; then
...
mysql -u ${sqlRootUser} -p${sqlRootPass} -e "GRANT USAGE ON * . * TO  '${sqlUser}'@'localhost' IDENTIFIED BY  '${sqlPass}' WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0 ;"
if [ "${?}" -eq "0" ]; then
...
mysql -u ${sqlRootUser} -p${sqlRootPass} -e "CREATE DATABASE IF NOT EXISTS ${sqlDBname} ;"
if [ "${?}" -eq "0" ]; then
...
mysql -u ${sqlRootUser} -p${sqlRootPass} -e "GRANT ALL PRIVILEGES ON  ${sqlDBname} . * TO  '${sqlDBname}'@'localhost';"
if [ "${?}" -eq "0" ]; then
